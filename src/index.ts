...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'HERE');
}
...
const tools: McpToolExport['tools'] = [
  {
    ...
    summary: 'The coordinates for an address or place name, geocoded by HERE.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        radius_km: { type: 'number', description: 'Only return places within this many km of `near` (default 100, max 1000). HERE ranks by relevance, so without this a query that matches a business NAME can return that business on another continent.' },
      },
      required: ['query', 'near'],
    },
  },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  switch (name) {
    ...
    case 'here_discover': {
      ...
      // HERE's `at` biases ranking; it does not bound it. "top attractions"
      // near Cox's Bazar (21.44,91.99) came back as the Burj Khalifa, 3,760 km
      // away, and a UK marketing firm called "Top Attractions", 8,246 km away —
      // a confident list of the wrong places (fleet #2302). Enforce the radius
      // here and SAY how many were dropped, so an empty result reads as "none
      // nearby matched that phrasing", not as a clean zero.
      const radiusKm = Math.min(1000, Math.max(1, Number(args.radius_km) || 100));
      const places = items.filter((i) => i.distance_m == null || i.distance_m <= radiusKm * 1000);
      const dropped = items.length - places.length;
      return {
        query, near: at, radius_km: radiusKm, count: places.length, places,
        ...(dropped > 0 ? {
          dropped_beyond_radius: dropped,
          note: `${dropped} match(es) farther than ${radiusKm} km from ${at} were removed — HERE matched the query text to places elsewhere. `
            + (places.length === 0 ? 'Try a category word (e.g. "tourist attraction", "restaurant") instead of a phrase, or raise radius_km.' : ''),
        } : {}),
      };
    }
    case 'here_route': {
